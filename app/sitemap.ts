...
import { absoluteUrl, getNotePath, getPyq2020Path, getPyq2021Path, getPyq2022Path, getPyq2023Path, getPyq2024Path, getPyq2025Path, getQuestionPath, getQuestions, getSeoNotes, getTopicPath, getTopics } from "@/lib/content";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    { url: absoluteUrl(getPyq2023Path()), lastModified: now, changeFrequency: "weekly", priority: 0.86 },
    { url: absoluteUrl(getPyq2022Path()), lastModified: now, changeFrequency: "weekly", priority: 0.84 },
    { url: absoluteUrl(getPyq2021Path()), lastModified: now, changeFrequency: "weekly", priority: 0.83 },
    { url: absoluteUrl(getPyq2020Path()), lastModified: now, changeFrequency: "weekly", priority: 0.82 },
