...
import { getSeoNotes, getTopics } from "@/lib/content";
...
export default async function SiteMapPage() {
  ...
  return (
    <main className="page">
      ...
      <section className="contentBand"><h2>Core pages</h2><div className="miniLinks"><Link href="/">MedQGo NEET Biology revision notes</Link><Link href="/neet-ug/biology">NEET Biology revision library</Link><Link href="/neet-biology-pdf">Revision workbook early access</Link><Link href="/about">About MedQGo</Link><Link href="/contact">Contact</Link><Link href="/privacy">Privacy Policy</Link><Link href="/terms">Terms of Use</Link><Link href="/team">Editorial Team</Link><Link href="/authors/dongfeng">Author: DongFeng</Link><Link href="/editorial-policy">Editorial Policy</Link><Link href="/copyright">Copyright</Link></div></section>
      <section className="contentBand"><h2>In-depth revision guides</h2><div className="miniLinks">{topics.map((topic) => <Link href={`/neet-ug/biology/${topic.slug}`} key={topic.id}>{topic.name} revision guide</Link>)}</div></section>
    </main>
  );
}
